...
func NewContext(ctx context.Context, l *lexer.Lexer) *Parser {
	...
	p.registerInfix(lexer.PLUS_ASSIGN, p.parseInfixOperatorAssignment("+"))
	p.registerInfix(lexer.MINUS_ASSIGN, p.parseInfixOperatorAssignment("-"))
	p.registerInfix(lexer.SLASH_ASSIGN, p.parseInfixOperatorAssignment("/"))
	p.registerInfix(lexer.ASTERISK_ASSIGN, p.parseInfixOperatorAssignment("*"))
...
func (p *Parser) parseAsInfix(left ast.Expression, precedence int) ast.Expression {
	for !p.peekTokenIs(lexer.SEMICOLON) && !p.newlineBeforeStatementModifier() && precedence < p.peekPrecedence() {
...
func (p *Parser) newlineBeforeStatementModifier() bool {
	return p.curTokenIs(lexer.NEWLINE) && p.peekTokenIsMultiple(lexer.IF, lexer.UNLESS, lexer.WHILE)
}
